...
- 日程与任务：日程继续来自 Apple Calendar，任务复用 Markdown 与 Apple Reminders；右侧任务可拖到日 / 周时间轴安排日期和具体时间，定时任务使用带复选框的轻量虚线标记。今天完成的任务会保留在原时间位置，无具体时间的完成项显示在时间轴顶部，今天摘要同步显示完成数量。点击任务可在右侧原位编辑日期、时间、提醒、优先级、重复、项目、标签、备注和关联笔记；项目同时筛选统一工作台的任务列表与时间轴。快速任务输入支持无 AI 的中文日期、时间、提前提醒、标签和优先级解析，确认预览后才创建。重复任务兼容 Obsidian Tasks，并在完成后保留历史、生成下一次。桌面三栏可折叠并记住宽度，移动端使用今天、任务、日历单栏切换。
