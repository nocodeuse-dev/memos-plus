...
- 日程与任务：日程继续来自 Apple Calendar，任务复用 Markdown 与 Apple Reminders；右侧任务可拖到日 / 周时间轴安排日期和具体时间，定时任务使用带复选框的轻量虚线标记。点击任务可在右侧原位编辑日期、时间、提醒、优先级、项目、标签、备注和关联笔记；项目只作为统一工作台筛选条件。桌面三栏可折叠并记住宽度，移动端使用今天、任务、日历单栏切换。
