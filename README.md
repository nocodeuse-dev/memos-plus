...
- 即时归档：把内容直接投递到项目文件、标签文件、最近文件或真实 Markdown 标题下；发送窗口可独立勾选“作为任务”，将任意内容送入现有任务设置并同步到 Apple Reminders，而不要求目标必须是项目。
...
- 统一工作台：主页目录与“日程与任务”现在共享同一个长期存在的左侧栏和右侧内容区；目录、任务、学习、项目只替换右侧内容，不会再创建第二套侧栏结构。侧栏顶部固定显示笔记/标签/天数/今天/待办统计和迷你日历，其他导航独立滚动；桌面端可折叠、调宽并记住滚动位置，移动端按单列顺序阅读和切换。任务树按今天、进行中、时间、异常、归档分组，低频组默认收起并保留每项数量与直接筛选；进行中、等待中、已延期、已取消兼容同名 Markdown 标签。目录继续显示笔记、目录、检索式与自定义筛选；学习提供今日复习、待复习、学习中、需要加强和已掌握；项目只作为现有任务与时间轴的筛选维度。日程继续来自 Apple Calendar，任务复用 Markdown 与 Apple Reminders；右侧任务可拖到日 / 周时间轴安排日期和具体时间，定时任务使用带复选框的轻量虚线标记。点击完成会保留 Tasks 兼容的完成日期及独立的真实完成时间；今天完成的任务会保留在原时间位置，无具体时间的完成项显示在时间轴顶部，今天摘要同步显示完成数量。点击任务可在右侧原位编辑日期、时间、提醒、优先级、重复、项目、标签、备注和关联笔记；Markdown 正文中的任务行也可通过“编辑当前任务”命令或右键“编辑任务”打开同一编辑器。首页、工作台和快速任务面板共用无 AI 的中文任务输入核心，统一解析绝对/相对日期、时间范围、提前提醒、标签、优先级和重复规则，并可通过智能发送/搜索选择任意文件与标题；缺少具体日期的“下周 / 明年3月 / 2028年”等表达会保留原文并要求确认。重复任务兼容 Obsidian Tasks，并在完成后保留历史、生成下一次。
- 学习复习卡：收录内容带 `#学习` 时，在 Markdown 写入成功后创建来源关联的轻量复习卡；“日程与任务”内可按今日复习、待复习、学习中、需要加强、已掌握和全部复习卡查看。学习模式采用独立的轻量 FSRS 调度器，根据“忘记 / 困难 / 记住 / 简单”和实际复习间隔动态安排下一次复习。卡片打开时优先读取原文件和标题，来源失效才回退小型快照；不建立全文索引、不引入 Anki 数据库，也不影响 Apple 同步。
- 快速任务面板：右下角任务图标打开紧凑任务面板，可快速添加任务并查看今天、近 7 天、重要和逾期任务；创建时复用首页相同的任务字段、发送位置、Markdown 写入、TaskIndex 和 Apple 同步链路，完成、打开来源和任务设置也继续使用现有任务系统。桌面端显示在状态栏上方，移动端使用底部抽屉，面板内可进入完整“日程与任务”工作台。
