...
- 日程与任务：日程继续来自 Apple Calendar，任务复用 Markdown 与 Apple Reminders；任务列表会显示日期、具体时间、优先级和 Apple 同步状态，带具体时间的任务也会以轻量标记出现在当天时间轴。状态栏、今天、全部任务、优先级和项目任务入口统一打开这个工作台，并支持关键词、优先级与项目筛选。顶部刷新会同时刷新日程和任务。
