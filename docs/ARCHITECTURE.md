...
- 发送到文件：支持智能发送、按标签及全库文件名/路径搜索选择 Markdown 文件，再选择标题插入；智能发送只从待发送文字提取轻量关键词并复用现有搜索缓存，不访问网页、不调用 AI 或建立新索引。“作为任务”是独立于目标文件/标题的单次发送意图，勾选后复用现有任务表单和写入/Apple 同步链路，未勾选时继续使用普通发送。
...
- 统一工作台导航：`src/workbenchNavigation.ts` 是目录、任务、学习和项目的唯一导航声明；`MemosPlusView` 是唯一持有 Sidebar + Content Area 的工作台壳。目录、任务、学习、项目只替换右侧内容模块，绝不再创建另一套功能侧栏或通过 `setViewState` 在同一标签间切换两个完整 ItemView。目录继续复用主页的 memo/目录/检索式/自定义筛选；任务、学习、项目只转发到既有 TaskIndex、学习卡和时间轴组件，不创建第二套数据源。
- 日程与任务工作台：`TaskCalendarSurface` 只作为统一壳右侧的日程、任务和学习内容模块；迷你日历由 `MemosPlusView` 的唯一固定顶部区创建，`TaskCalendarSurface` 只向固定区提供可选的日历来源列表，绝不在项目或其他滚动内容中创建第二份日历。左栏拆为固定顶部区（统计概览和迷你日历）与独立滚动内容区（目录、任务、学习、项目等导航）；高度不足时仅日历可收缩，宽度、折叠和滚动位置沿用同一组设置。任务树按今天、进行中、时间、异常、归档分组，复用 TaskIndex 的日期、创建日期、优先级、完成信息以及任务行状态标签；仅是本地筛选，不建立第二套索引。旧 `TaskCalendarView` 只保留为历史 WorkspaceLeaf 的迁移重定向。按当前日/周只读显示默认常用或用户指定的 Apple 日历日程，并复用全库 Markdown `TaskIndex` 管理待处理、今日新增、逾期和已完成任务；项目只作为任务列表与时间轴共用的筛选条件。右侧任务可拖到时间轴排期，点击后在同一右栏编辑既有 Markdown 任务；快速输入调用共享 `UnifiedTaskComposer` 解析日期、时间、提醒、标签和优先级，并可复用现有文件/标题目标选择。共享任务弹窗由外层 shell 统一限制宽度，发送路径和目标操作分层自适应布局，内容区仅纵向滚动；不建立第二套任务数据库，也不改写 Apple 同步桥接层。
...
| 日程与任务工作台 | `main.ts`, `src/view.ts`, `src/taskCalendar.ts`, `src/taskCalendarAgendaGrid.ts`, `src/taskCalendarTaskEditor.ts`, `src/taskCalendarEditSession.ts`, `src/taskNaturalLanguage.ts`, `src/taskCalendarView.ts`, `src/appleCalendarAgenda.ts`, `src/taskCalendarEventModal.ts`, `src/taskCalendarEventDetailModal.ts`, `src/taskActions.ts`, `src/taskNavigation.ts` | `MemosPlusView`, `TaskCalendarSurface`, `TaskCalendarEditSession`, `parseNaturalLanguageTask`, `taskCalendarTaskWithPatch`, `taskCalendarDropTime`, `updateTaskCalendarTaskLine`, `TaskCalendarEventModal`, `AppleCalendarAgendaService`, `taskCalendarTasks` | 唯一 `MemosPlusView` WorkspaceLeaf 和唯一常驻侧栏；Ribbon、命令和历史日程标签均路由到同一壳。`TaskCalendarSurface` 只替换右侧内容，迷你日历、项目和日历筛选复用左栏并保存同一宽度、折叠状态与滚动位置。首次打开立即渲染基础界面，不等待项目扫描或 Calendar 读取；同步异常只显示局部状态。桌面端使用侧栏 / 日程 / 任务布局；移动端单列展示。任务唯一来源仍是 `TaskIndex` 与 Markdown，项目筛选同时约束右侧列表和中间时间轴。右侧编辑使用短生命周期乐观会话：字段先在本地快照即时生效，连续补丁串行写入 Markdown，保存失败保留输入并可重试；TaskIndex 和 Apple 后台通知不得销毁正在编辑的表单。快速输入的本地规则解析只显示预览，确认后才复用任务格式写入。Apple 同步只在 Markdown 保存完成后异步运行，并独立更新同步状态。右侧任务可拖到时间轴按 15 分钟排期，定时任务以带复选框的虚线标记和 Calendar 色块区分。日历读取仍使用当前日/周有界查询和运行期缓存。 |
...
| Apple 双向同步 | `main.ts`, `src/appleSync.ts`, `src/appleSyncBridge.ts`, `src/appleJxaRunner.ts`, `src/appleSyncService.ts`, `src/settings.ts` | `AppleSyncService`, `MacOsAppleSyncBridge`, `runAppleJxa`, `resolveAppleSyncDirection`, `updateTaskLineFromApple` | 默认关闭，仅 macOS 桌面端运行；普通任务同步 Apple Reminders，区分截止日期时间与独立提醒时间，并保留完成状态和已关联项目的双向删除。统一表单明确选择 Calendar 的任务使用开始/结束时间、全天、提醒和重复字段执行幂等写入；日历工作台继续按当前日/周读取 Calendar。JXA 程序与请求通过标准输入交给固定参数的 `osascript`，不把任务内容放进 argv；隐藏任务元数据在同步前兼容清理遗留空格形式并收敛为唯一标准标记，防止 Markdown、Apple 标题和同步签名随往返操作膨胀。隐藏本地 ID、Apple 唯一 ID 和上次签名防止重复，旧的无目标元数据任务仍按 Reminders 规则兼容。 |
...
| 新建文件模板库 | `src/fileTemplateLibrary.ts`, `src/projectFileSuggestModal.ts`, `src/mobilePanelView.ts`, `src/store.ts`, `src/settings.ts` | `FileTemplateLibraryItem`, `scanFileTemplateLibrary`, `FileTemplateLibraryModal`, `createFileFromLibraryTemplate` | 搜索不到目标文件时使用；Store 层只递归遍历配置的模板文件夹，不建立全库索引，模板内容本身只负责新文件骨架，和输入格式模板分离。 |
...
| 缓存 / 索引 / 性能日志 | `src/performance.ts`, `src/vaultIndex.ts`, `src/vaultSearch.ts`, `src/view.ts` | `PerformanceProfiler`, `VaultMetadataIndex`, `VaultSavedSearchIndex`, `vaultSearchCacheKeys` | 项目、标签文件和 vault metadata 检索复用统一索引；首次全库索引在后台分批建立并按帧让出主线程。正文/任务条件由 `VaultSavedSearchIndex` 按需读正文并缓存文件内容，混合检索会先做 metadata 预筛选；模板库单独使用目录级扫描。 |
...
- `TasksMarkdownOptions`：优先级、项目标签、开始/计划/截止日期、具体时间、结束时间、独立提醒、全天、重复、创建日期和同步目标。Tasks 可识别字段保持原语法，Apple 专用字段使用唯一的 `memos-plus-task-meta` 隐藏注释保存；解析兼容历史 `memos-plus-task- meta` 空格形式，任何写回都会删除重复项并规范化为一个标准标记。
