...
- 标题
- 完成/未完成
- Apple 完成日期，写回为 Obsidian Tasks 兼容的 `✅ YYYY-MM-DD`
- 截止日期；没有截止日期时使用计划日期
...
工作台快速新增任务在同步已启用时会自动加入同步标签。顶部刷新按钮会同时刷新 Calendar 日程和设置中当前选中的 Reminder List；两个请求保持独立，任何一边失败都不会把任务写成日历事件。Reminders 失败时工作台会显示系统返回的列表、权限、超时或 iCloud 等具体原因；暂时未返回仍按“等待同步”重试，不直接当成永久失败。
