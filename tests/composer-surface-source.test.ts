...
describe("composer surface routing", () => {
  it("passes an explicit surface from every shared composer entry point", () => {
    ...
    expect(viewSource).toContain('this.renderComposer(main, activeSurface === "mobile" ? "mobileHome" : "home", modules, initialComposerContent)');
    expect(viewSource).toContain('this.renderComposer(home, "mobileHome", modules, initialComposerContent)');
